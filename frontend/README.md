...
assets/
  css/site.css          전체 스타일
  js/voyage.js          나룻배 항해 장면 — 작은 3D 투영 + 캔버스 (스크롤 연동)
  js/projects.js        관리자 화면에서 공개한 프로젝트를 Projects 섹션에 그림
  js/site.js            스크롤 진행도 · 챕터 전환 · 탭 · 등장 애니메이션
  img/                  프로필 · 스크린샷
  projects/dongtan/     동탄 조례 지표판 과정물 (보고서·CSV·사전 조사 PDF)
cbd-dashboard/          프로젝트 01 — 아파트 밀도 CBD 탐색기 (실행 가능한 데모)
profile/                프로필 페이지 (헤더·소개·연락처에서 링크, 검색 노출은 noindex)
admin/                  관리자 화면 (백엔드 필요)
```

## 🌊 항해 인트로

나룻배(사공·삿갓·노·뜸·이물 등불)와 동행 배 셋을 작은 3D 모델로 만들고, 카메라 하나가 스크롤 진행도(0 → 1)를 따라 끊기지 않는 경로로 움직입니다.
장면을 갈아 끼우지 않아 옆모습에서 배 위, 상공까지 한 번의 카메라 이동으로 이어집니다. 영상 파일 없이 전부 코드로 그립니다.

| 구간 | 카메라 | 화면 |
|---|---|---|
| 0.00 – 0.14 | 오른쪽 옆 | 잔잔한 바다 위의 나룻배와 동행 배 · 대표 문구 |
| 0.14 – 0.38 | 뱃머리 쪽으로 돌며 접근 | 배를 계속 바라보며 다가가 뱃전 위로 |
| 0.38 – 0.64 | 앞자리에 앉음 | 고개를 들어 뱃머리 너머를 봄, 배와 함께 흔들림 · 살펴보는 것들 |
| 0.64 – 1.00 | 뒤로 물러나며 상승 | 배 전체와 항적을 내려다봄 · 한 줄 자기소개 |

카메라 위치·시선은 단조 3차 보간으로 이어, 키프레임을 지날 때 멈칫하지 않고 물속으로 넘치지도 않습니다.
이후 프로젝트 쇼케이스(탭) → 프로젝트 상세 → 이력 → 연락처가 이어집니다. 상단 우측 **Skip intro**로 인트로를 건너뛸 수 있습니다.

## 🗂 프로젝트

| 번호 | 프로젝트 | 링크 |
|---|---|---|
| 01 | 아파트 밀도 CBD 탐색기 | `cbd-dashboard/` |
| 02 | 화성시 동탄구 조례 지표판 (팀 LUOGO) | <https://cupcakegroundused.vercel.app> · 과정물 `assets/projects/dongtan/` |
| 03~ | 관리자 화면에서 공개한 프로젝트 | `#projects` 섹션에 자동 표시 |

새 프로젝트를 직접 페이지에 넣을 때는 쇼케이스 패널·탭(`.panel`, `.tabs`), 우측 레일, 상세 섹션을 함께 추가하고 `#projects`의 `data-start` 번호를 하나 올립니다.
